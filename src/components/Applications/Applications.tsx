...
const Applications = () => {
	...
	function getApiData(pageNumber: number, numberToLoad: number) {
		...
		axios
			.get(
				`http://localhost:3001/api/applications?_page=${pageNumber}&_limit=${numberToLoad}`
			)
			.then(res => {
	...
	return (
		<div>
			{isError ? (
				<ErrorModal isError={isError} onClose={() => {setIsError(false)
					console.log(isError)}} />
			) : null}
			{/* Any way to render this without the operator? */}
